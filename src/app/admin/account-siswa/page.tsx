"use client";

import React, { useState, useEffect } from "react";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Alert, AlertDescription } from "@/components/ui/alert";
import { Skeleton } from "@/components/ui/skeleton";
import { AccountCredentialsForm } from "@/components/account-credentials-form";
import { GraduationCap, Search, AlertCircle } from "lucide-react";

interface SiswaData {
  id: string;
  email: string;
  username: string;
  fullName: string;
  role: string;
}

export default function SiswaAccountPage() {
  const [siswa, setSiswa] = useState<SiswaData | null>(null);
  const [siswaList, setSiswaList] = useState<SiswaData[]>([]);
  const [selectedSiswa, setSelectedSiswa] = useState<SiswaData | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [searchTerm, setSearchTerm] = useState("");

  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch current user
        const meResponse = await fetch("/api/users/me", {
          method: "GET",
          credentials: "include",
        });

        const meResult = await meResponse.json();

        if (!meResponse.ok) {
          setError(meResult.message || "Gagal memuat data");
          setLoading(false);
          return;
        }

        setSiswa(meResult.data);

        // Fetch siswa list (admin only feature)
        if (meResult.data.role === "SUPER_ADMIN") {
          const siswaResponse = await fetch("/api/users?role=SISWA", {
            method: "GET",
            credentials: "include",
          });

          const siswaResult = await siswaResponse.json();
          if (siswaResponse.ok && siswaResult.data) {
            setSiswaList(siswaResult.data);
          }
        }

        setError("");
      } catch (err) {
        setError("Terjadi kesalahan saat memuat data");
      } finally {
        setLoading(false);
      }
    };

    fetchData();
  }, []);

  const filteredSiswa = siswaList.filter(
    (s) =>
      s.fullName.toLowerCase().includes(searchTerm.toLowerCase()) ||
      s.email.toLowerCase().includes(searchTerm.toLowerCase()) ||
      s.username.toLowerCase().includes(searchTerm.toLowerCase()),
  );

  const isAdmin = siswa?.role === "SUPER_ADMIN";
  const editingSiswa = selectedSiswa || siswa;

  if (loading) {
    return (
      <div className="space-y-6">
        <Skeleton className="h-16 w-full" />
        <div className="grid gap-4">
          <Skeleton className="h-10" />
          <Skeleton className="h-40" />
        </div>
      </div>
    );
  }

  if (error) {
    return (
      <Alert variant="destructive">
        <AlertCircle className="h-4 w-4" />
        <AlertDescription>{error}</AlertDescription>
      </Alert>
    );
  }

  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="bg-gradient-to-r from-purple-900 to-purple-800 text-white p-6 rounded-lg">
        <div className="flex items-center gap-3 mb-2">
          <GraduationCap size={28} />
          <h1 className="text-3xl font-bold">Manajemen Akun Siswa</h1>
        </div>
        <p className="text-purple-100">
          {isAdmin
            ? "Kelola akun siswa peserta didik"
            : "Kelola akun Anda sebagai siswa"}
        </p>
      </div>

      {/* Admin View - Siswa List */}
      {isAdmin && (
        <>
          {/* Search */}
          <Card className="border border-slate-200">
            <CardHeader className="bg-slate-50">
              <CardTitle className="text-lg">Daftar Siswa</CardTitle>
            </CardHeader>
            <CardContent className="pt-6">
              <div className="flex gap-2 mb-4">
                <div className="relative flex-1">
                  <Search
                    className="absolute left-3 top-3 text-slate-400"
                    size={18}
                  />
                  <Input
                    placeholder="Cari nama, email, atau username siswa..."
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                    className="pl-10"
                  />
                </div>
              </div>

              {filteredSiswa.length === 0 ? (
                <p className="text-center text-slate-600 py-8">
                  Tidak ada siswa ditemukan
                </p>
              ) : (
                <div className="space-y-2">
                  {filteredSiswa.map((s) => (
                    <div
                      key={s.id}
                      className={`p-4 border rounded-lg cursor-pointer transition ${
                        selectedSiswa?.id === s.id
                          ? "border-purple-500 bg-purple-50"
                          : "border-slate-200 hover:border-purple-300"
                      }`}
                      onClick={() => setSelectedSiswa(s)}
                    >
                      <p className="font-semibold text-slate-900">
                        {s.fullName}
                      </p>
                      <p className="text-sm text-slate-600">
                        {s.email} • {s.username}
                      </p>
                    </div>
                  ))}
                </div>
              )}
            </CardContent>
          </Card>
        </>
      )}

      {/* Own Account Info */}
      {!isAdmin && siswa && (
        <Card className="border border-slate-200">
          <CardHeader className="bg-purple-50">
            <CardTitle className="text-lg">Informasi Akun Anda</CardTitle>
          </CardHeader>
          <CardContent className="pt-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="text-sm font-semibold text-slate-600">
                  Nama Lengkap
                </label>
                <p className="text-lg font-semibold text-slate-900 mt-1">
                  {siswa.fullName}
                </p>
              </div>
              <div>
                <label className="text-sm font-semibold text-slate-600">
                  Email
                </label>
                <p className="text-base text-slate-900 mt-1">{siswa.email}</p>
              </div>
              <div>
                <label className="text-sm font-semibold text-slate-600">
                  Username
                </label>
                <p className="text-base text-slate-900 mt-1">
                  {siswa.username}
                </p>
              </div>
              <div>
                <label className="text-sm font-semibold text-slate-600">
                  Role
                </label>
                <div className="mt-1">
                  <span className="inline-block px-3 py-1 bg-purple-100 text-purple-800 rounded-full text-sm font-semibold">
                    Siswa / Peserta Didik
                  </span>
                </div>
              </div>
            </div>
          </CardContent>
        </Card>
      )}

      {/* Credentials Form */}
      {editingSiswa && (
        <div>
          {isAdmin && selectedSiswa && (
            <div className="mb-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
              <p className="text-sm text-blue-900">
                Mengedit akun:{" "}
                <span className="font-semibold">{selectedSiswa.fullName}</span>
              </p>
            </div>
          )}

          <AccountCredentialsForm
            userId={editingSiswa.id}
            currentEmail={editingSiswa.email}
            currentUsername={editingSiswa.username}
            onSuccess={() => {
              if (isAdmin && selectedSiswa) {
                setSelectedSiswa(null);
              }
            }}
          />
        </div>
      )}
    </div>
  );
}
